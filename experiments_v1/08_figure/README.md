...
**Key Finding**: System exhibits binary regime switching—33% of seeds use warmup expert (n_eff matters, +4.6% effect), 67% use tabula rasa expert (n_eff ignored). This regime-dependent behavior cross-validates with Figure 7 (N=30 seeds: 30% warmup / 70% tabula rasa).
...
2. **Cross-validation**: Figure 7 provides robust estimates (N=30 seeds)
...
**Cross-Validation**:
- Figure 7 (N=30): 30% warmup-dominant / 70% tabula rasa-dominant
...
**Figure 7 (Zero-Shot Model Adoption, N=30 seeds)**:
...
2. **Cross-validation**: Figure 7 (N=30) provides robust population estimates
3. **Binary outcomes**: All 3 seeds show clear expert commitment
4. **Consistency check**: Proportions match Figure 7 (33% vs 30%)
...
**Primary Evidence**: Figure 7 (N=30 seeds)
...
### Figure 7: Zero-Shot Model Adoption (N=30)
...
**Connection**: This experiment confirms binary switching mechanism observed in Figure 7
...
**Mitigation**: Figure 7 provides robust estimates (N=30)
...
- **Table 2** (`experiments_v1/02_table/`): Multi-seed validation foundation
- **Figure 7** (`experiments_v1/07_figure/`): Primary regime proportion evidence (N=30)
- **Figure 6** (`experiments_v1/06_figure/`): Safety regime validation
...
Cross-Validation (Figure 7, N=30 seeds):
...
3. **Cross-Validation** → Matches Figure 7 (30%/70%, N=30 seeds)
