"""
Diagnostic: Check if Figure 7's "75% warmup" claim hides regime switching
===========================================================================
Run Figure 7 experiment with expert weight tracking to see if 75/25 is:
...
TOTAL_STEPS = 800
RELEASE_STEP = 300
SEEDS_TO_CHECK = [42, 43, 44, 45, 46]  # First 5 seeds from Figure 7's range (42-71)
...
def run_figure7_config(seed: int):
    ...
    # Figure 7 configuration: use_corralling=True, eta=0.1
    router = BanditRouter.create(
        ...
        use_corralling=True,
        corralling_learning_rate=0.1,  # Figure 7 uses conservative eta
...
if __name__ == "__main__":
    logger.info("\n" + "="*80)
    logger.info("DIAGNOSTIC: Figure 7 Expert Weight Analysis")
    logger.info("="*80)
    ...
    if binary_seeds > blended_seeds:
        logger.info("\n⚠️ WARNING: Figure 7's '~75% warmup' likely hides REGIME SWITCHING!")
        ...
        logger.info("\n✓ Figure 7's '~75% warmup' represents TRUE BLENDED weights")
    ...
    logger.info("\n" + "="*80)
    logger.info("\n✅ Diagnostic complete. Check if Figure 7 and 8 are consistent.\n")
