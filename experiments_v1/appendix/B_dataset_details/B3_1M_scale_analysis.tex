...
\item \textbf{Economic Implication:} The shift from 17.6\% to 5.9\% hard prompts means that warmup priors biased toward expensive flagship models would waste budget on an additional 11.7\% of production traffic if priors were harmful. However, Table~\ref{tab:performance_gap} shows that under the moderate shift in our evaluation, warmup priors remain beneficial. The economic impact of misrouting depends on the severity of domain mismatch in each deployment; the values here illustrate the \emph{potential} scale of the routing opportunity, not a measured savings.

\item \textbf{Implications for Table 2:} Under the moderate distribution shift evaluated in Table~\ref{tab:performance_gap}, warmup priors remain beneficial (28\% lower regret than tabula rasa). The 94.1\% routine traffic dominance at production scale suggests that priors calibrated for general LLM traffic will transfer well to most deployments. For deployments with more severe domain mismatch (e.g., specialized medical or legal traffic), Corralling provides bounded-cost insurance (1--4\% overhead) while online learning continuously refines the routing policy.
\end{tablenotes}
\end{table}
...
The 317$\times$ scale validation demonstrates that findings generalize from controlled experiments to production-scale deployments. This validation is significant because:
...
This appendix positions the work as \emph{production-ready} and \emph{theoretically grounded}, demonstrating both empirical rigor and practical impact for industry-scale machine learning systems.
