...
\textbf{(Top-right, $\lambda=0.1$)} Mild cost awareness: GPT-4o still dominant (74.1\%, quality=96.0\%, \$9.74/M).
\textbf{(Bottom-left, $\lambda=0.3$)} Moderate: GPT-4o dominant (74.7\%, quality=95.6\%, \$9.25/M).
\textbf{(Bottom-right, $\lambda=0.5$)} Aggressive cost preference: Mixtral rises to 28.6\%, GPT-4o drops to 56.5\% (quality=91.7\%, \$6.67/M).
%
All values are 5-seed averages (seeds 42--46).
Critically, cost monotonically decreases with $\lambda$ (\$11.07 $\to$ \$6.67/M) while quality degrades gracefully (95.5\% $\to$ 91.7\%), validating the composite reward as an effective lever for the cost-quality tradeoff.
The high variance at $\lambda=0.5$ ($\pm$20.4\% Mixtral usage) reflects that aggressive cost preference amplifies exploration dynamics with limited Phase~2 data (N=561).
}
