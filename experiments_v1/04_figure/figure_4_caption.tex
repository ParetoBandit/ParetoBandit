...
The algorithm initially relies on warmup priors (orange) but shifts weight to tabula rasa (green) after discovering that the warmup expert's bias toward expensive flagship models (GPT-4-Turbo, Claude-3) yields lower quality on the Easy cluster.
...
The warmup expert, trained on RouteLLM data emphasizing quality, exhibits an ``expensive bias''---overreliance on flagship models (GPT-4-Turbo, Claude-3) even when they provide no quality advantage.
