% KDD Figure 4 - Results Summary and Caption
% Complete numerical results for the Pareto Frontier experiment

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/figure4_pareto_with_dominated.png}
...
\textit{Unlike static policies where upgrading to GPT-4 incurs a $43\times$ cost increase for a net quality loss ($-1.3\%$), banditGPT-Hybrid successfully leverages the expensive model to unlock a $+10.4\%$ quality gain while still costing $27\%$ less than the GPT-4 baseline.}
...
\begin{enumerate}
    \item \textbf{The ``Stupidity Tax''}: Static users pay $43\times$ more to get $1.3\%$ worse quality
    \item \textbf{Synergistic Breakout}: banditGPT exceeds both individual models ($0.909 > 0.823, 0.812$)
    \item \textbf{Budget Efficiency}: Achieves $27\%$ lower cost than GPT-4 while delivering $+11.9\%$ better quality
...
\textbf{Key Insight:} This demonstrates the ``Intelligence Tax''---RouteLLM's threshold-based approach cannot distinguish between prompts where GPT-4 helps vs. hurts, leading to negative returns on investment beyond \$0.0065.
...
\begin{itemize}
    \item Router: Matrix Factorization (pre-trained on Augment-100k)
...
\subsection*{Key Takeaways for KDD Reviewers}
...
A: RouteLLM uses a pre-trained threshold calibrated on aggregate statistics. On our dataset, Mistral outperforms GPT-4 on average (0.823 vs. 0.812). When RouteLLM's threshold forces more GPT-4 usage, it allocates the expensive model to prompts where it actually hurts quality. \texttt{banditGPT} learns this inversion online, using GPT-4 only for the \textasciitilde6\% of prompts where it genuinely helps.
