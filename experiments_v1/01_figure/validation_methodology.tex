...
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
    \item \textbf{$\chi^2$ test of independence:} $p < 0.0001$ (outcome proportions differ significantly between PC1 regions)
    \item \textbf{Cram\'er's $V$:} Reported in figure (categorical effect size)
    \item \textbf{Contingency table:} Win/tie/loss counts by PC1 region (see figure panel B)
\end{itemize}
...
Our analysis involves multiple tests: 50 candidate thresholds in the silhouette search, 40 thresholds in the stability sweep, two PCA approaches, and multiple test statistics ($\chi^2$, Mann-Whitney $U$, Cram\'er's $V$, Cohen's $d$). We do not apply formal correction (e.g., Bonferroni) for the following reasons: (1)~the primary threshold is selected by an unsupervised criterion (silhouette score) with no reference to reward labels or $p$-values, so no multiplicity adjustment is needed for the primary analysis; (2)~the threshold sweep is explicitly a \emph{sensitivity analysis}, not a search for the ``best'' $p$-value---we report effect size (\emph{Cram\'er's $V$}) across all thresholds precisely because $p$-values are uninformative at this sample size; and (3)~the two PCA approaches test a specific methodological hypothesis (domain adaptation vs.\ generic baseline), not a fishing expedition. We acknowledge that the overall analysis involves researcher degrees of freedom (choice of embedding model, PCA dimensionality, clustering method), and encourage readers to evaluate the generic PCA result ($d = 0.33$) as the conservative baseline.
...
\begin{enumerate}[leftmargin=*,noitemsep,topsep=0pt]
    ...
    \item \textbf{Data quality:} Zero duplicates; High PC1 region is homogeneous (diversity $= 0.355$); region content characterized via TF-IDF analysis
    \item \textbf{Data integrity:} Held-out only ($N=750$); dev set excluded
    \item \textbf{Generalizability:} Region proportion (19.2\% High PC1) is specific to the holdout; 1M dataset shows $\sim$5.9\% in corresponding spatial region
\end{enumerate}
