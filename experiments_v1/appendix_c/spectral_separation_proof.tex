...
\subsubsection{Meta-Algorithm Regret Guarantee}

The theoretical soundness of our $\eta=1.0$ meta-algorithm is established through the following regret bound:
\begin{equation}
...
This bound demonstrates sublinear regret growth with two key components:
\begin{itemize}
...
The choice of $\eta=1.0$ is optimal because it balances rapid adaptation (larger $\eta$ reduces the exploration term) with stability (smaller $\eta$ prevents oscillations in expert weights). Our empirical results in Section~5 confirm that $\eta=1.0$ achieves the best performance across all tested configurations.
...
The combination of spectral separation and the meta-algorithm's regret guarantee provides both theoretical and empirical justification for our approach. The router can reliably distinguish between prompts requiring different model capabilities, while the meta-algorithm efficiently learns to weight expert opinions without excessive exploration cost.
