...
\caption{\textbf{Adaptive Prior Decommissioning: Phased Stress Test.} 
We simulate a sudden distribution shift at $t=50$ to evaluate router agility.
\textbf{Phase 1 (Neutral Zone):} For $t < 50$, both models perform well. The router maintains uniform exploration ($\pi \approx 0.5$), validating that it does not prematurely collapse to a prior.
\textbf{Phase 2 (The Shift):} At $t=50$ (Blue Dashed Line), the environment shifts to "Alignment Tax" tasks where the Warmup Expert fails.
\textbf{The Reaction:} The system detects the loss divergence immediately. Within $\Delta t = 14$ steps, it triggers \textbf{Decisive Decommissioning} (Red Line), dropping the harmful prior to $<10\%$ weight. This proves the system acts as an automatic safety valve against production drift.}
