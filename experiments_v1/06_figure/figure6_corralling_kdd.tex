...
\textbf{Connection to Prior Work}: This experiment complements our analysis of long-term adaptation (Figure~\ref{fig:corralling_semantic}) and short-term semantic transfer (Figure~\ref{fig:accelerated-adoption}). While those experiments focus on adaptation timescales of 500--1,121 steps, this experiment tests \textbf{emergency response} in 3--50 steps. The warmup expert may contain semantic transfer priors (as in Figures~\ref{fig:corralling_semantic} and \ref{fig:accelerated-adoption}), and this experiment validates that catastrophic failure detection works even when such priors are incorrect.
...
\textbf{Three Operating Regimes}: This experiment demonstrates the \emph{safety regime} (3--50 steps, $\eta=0.3$), complementing the \emph{cold-start regime} (0--300 steps, $\eta=0.1$, Figure~\ref{fig:accelerated-adoption}) and \emph{convergence regime} (300--1,121 steps, $\eta=5.0$, Figure~\ref{fig:corralling_semantic}).
...
- \textbf{Recovery detection depends on learning rate regime:}
  \begin{itemize}
    \item $\eta=0.1$ (very conservative, Figure~\ref{fig:accelerated-adoption}): No recovery detection, $>$800 steps needed
...
\subsection{Connection to Semantic Transfer and Learning Rate Regimes}

\subsubsection{Three Operating Regimes Across Experiments}

Our experimental program (Figures~\ref{fig:corralling_semantic}, this figure, and \ref{fig:accelerated-adoption}) reveals three complementary operating regimes for Corralling with different learning rates:

\begin{table}[h]
\centering
\caption{Three Operating Regimes for Corralling-Based Systems}
...
Cold Start (Figure~\ref{fig:accelerated-adoption}) & $\eta=0.1$--$0.3$ & 0--300 steps & Exploit semantic transfer \\
...
\begin{itemize}
    ...
    \item \textbf{Short-term semantic transfer (Figure~\ref{fig:accelerated-adoption})} uses conservative learning ($\eta=0.1$) to exploit priors without rapid unlearning. Stable expert weights in that experiment do NOT validate semantic transfer correctness—they indicate the learning rate is too low to adapt. With $\eta=0.3$ (this experiment), some adaptation begins; with $\eta=5.0$ (Figure~\ref{fig:corralling_semantic}), complete unlearning occurs.
...
\subsubsection{Robustness to Incorrect Semantic Priors}

The warmup expert in this experiment may contain \textbf{semantic transfer} from related models (e.g., GPT-4o initialized from GPT-4-Turbo, as in Figure~\ref{fig:corralling_semantic}). Analysis in Figure~\ref{fig:accelerated-adoption} reveals that semantic similarity does \emph{not} predict performance correlation ($r=-0.38$, $p=0.75$). The mechanism is \textbf{implicit regularization} (breaking symmetry via initial variance), not semantic accuracy.
...
\begin{enumerate}
    ...
    \item We connect catastrophic failure detection (this figure, 3--50 steps) to the broader learning rate regime framework, including short-term semantic transfer exploitation (Figure~\ref{fig:accelerated-adoption}, $\eta=0.1$) and long-term convergence (Figure~\ref{fig:corralling_semantic}, $\eta=5.0$).
