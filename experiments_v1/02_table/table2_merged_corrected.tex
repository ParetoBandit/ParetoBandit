...
Aggressive learning ($\eta=1.0$) achieves \textbf{1.10$\times$ competitive performance} 
relative to the Tabula Rasa baseline, while providing strong robustness against harmful priors.
Statistical significance: ** $p < 0.01$, *** $p < 0.001$ (Bonferroni-corrected, $\alpha=0.0083$).}
...
\quad Tabula Rasa (No Prior) & -- & TBD $\pm$ TBD & TBD $\pm$ TBD & 1.00$\times$ & -- \\
\quad Warmup (Harmful) & -- & TBD $\pm$ TBD & TBD $\pm$ TBD & TBD$\times$ & \textit{Baseline} \\
\midrule
\multicolumn{6}{@{}l}{\textit{banditGPT-Hybrid (Ours)}} \\
\quad Conservative & 0.1 & TBD $\pm$ TBD & TBD $\pm$ TBD & TBD$\times$ & +TBD\% \\
\quad \textbf{Aggressive} & \textbf{1.0} & \textbf{TBD $\pm$ TBD***} & \textbf{TBD $\pm$ TBD***} & \textbf{TBD$\times$} & \textbf{+TBD\%} \\
\bottomrule
\end{tabular}
...
\item \textbf{Gap to Baseline:} Performance multiplier relative to Tabula Rasa (lower is better). A value of 1.10$\times$ means 10\% more regret than the no-prior baseline. This quantifies the cost of the Corralling meta-algorithm overhead. Values near 1.00$\times$ indicate competitive performance.
