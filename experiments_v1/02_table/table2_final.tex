...
An independent t-test comparing conservative ($\eta=0.1$, mean=45.2$\pm$7.9) 
and aggressive ($\eta=1.0$, mean=48.1$\pm$16.8) learning rates yields $p=0.63$ (Cohen's $d=-0.22$),
but with only $N=10$ seeds, statistical power to detect this effect size is $\approx$8--10\%, 
so we cannot conclude the conditions are equivalent (see power analysis in text).
...
\item \textbf{Statistical Comparison:} Independent t-test comparing $\eta=0.1$ vs $\eta=1.0$ yields $t=-0.49$, $p=0.63$, Cohen's $d=-0.22$. \textbf{Power analysis caveat:} With $N=10$ seeds per condition and the observed effect size $d=0.22$, post-hoc power is approximately 8--10\% (well below the 80\% convention). To detect an effect of this magnitude at 80\% power would require $N\approx160$+ seeds per condition. \textbf{We therefore cannot conclude that the conditions are equivalent}---only that our sample size is insufficient to distinguish them. The non-significant $p$-value reflects low statistical power, not evidence of no difference. The practical recommendation for $\eta=0.1$ rests on its substantially lower variance (CV=17\% vs 35\%), not on a claim of equivalent mean performance.
\end{itemize}

\vspace{0.5em}
\paragraph{Domain Mismatch Scenario.}
This experiment tests robustness under realistic distribution shift (see Table~\ref{tab:dataset} for data provenance). The warmup priors, trained on RouteLLM battles with different model pairs (Mixtral-8x7B-Instruct vs GPT-4-Turbo) and sampling procedures, exhibit harmful negative transfer when applied to the evaluation distribution: warmup-only routing achieves 79 regret—1.98$\times$ worse than the tabula rasa baseline (40 regret). This 2$\times$ performance degradation quantifies the risk of relying on misaligned historical data. Our meta-learning approach adapts effectively through online learning, achieving 41--48 regret across different learning rates. This 39--48\% improvement over warmup-only routing ($\frac{79-48}{79}$ to $\frac{79-41}{79}$) demonstrates the algorithm's ability to automatically detect distribution mismatch and recover from harmful priors. The near-baseline performance (1.03--1.20$\times$) validates that the adaptation overhead is minimal—we match the performance of starting fresh while providing strong safety guarantees.
