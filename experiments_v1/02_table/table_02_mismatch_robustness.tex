...
\begin{table}[t]
\caption{\textbf{Table 2: Mismatch \& Robustness.} Performance under severe domain mismatch (68.6\% → 13.7\% hard prompts), showing domain alignment metrics and regret breakdown. Warmup priors exhibit low alignment (0.42) and catastrophic early-phase regret (82 in first 500 samples). Corralling with η=1.0 provides adaptive robustness, achieving 57\% improvement over warmup failure while maintaining near-optimal performance (1.26× vs oracle).}
...
\midrule
Tabula Rasa & N/A & 19.2 & 43.0 & -- \\
 & (Agnostic) & (optimal) & (optimal) & \\
\midrule
Warmup Only & 0.48 & 81.9 & 126.0 & \textit{baseline} \\
 & (Mismatch) & (harmful) & (catastrophic) & \\
\midrule
\textbf{Hybrid (η=1.0)} & \textbf{Adaptive} & \textbf{24.1} & \textbf{54.0} & \textbf{-57.1\%} \\
...
\item \textbf{Initial Regret (0--500):} Cumulative regret in first 500 samples, where domain mismatch causes maximum damage. This \emph{critical window} accounts for 65\% of warmup's total regret (81.9/126) but only 45\% of tabula rasa's regret (19.2/43). Aggressive learning (η=1.0) enables fast adaptation: Corralling achieves 24.1 regret (only 25\% worse than optimal 19.2), compared to warmup's catastrophic 81.9 regret (4.3× worse).

\item \textbf{Final Regret (Total):} Cumulative regret across all 1,121 samples. Warmup's low alignment (0.42) causes 126 total regret—2.93× worse than optimal. Corralling achieves 54 regret (1.26× worse than optimal), demonstrating successful adaptation despite poor initial alignment.

\item \textbf{vs Warmup Improvement:} Percentage reduction in total regret compared to warmup baseline. Corralling provides 57.1\% improvement: $(126-54)/126 = 0.571$. This quantifies the \textbf{safety guarantee} against negative transfer from mismatched warmup priors.
...
\item \textbf{Experimental Setup:} 1,121 prompts from LMSYS Arena dev set. Models: Mixtral-8x7B-Instruct vs GPT-4-Turbo. Warmup trained on RouteLLM (68.6\% hard: coding, math) → Evaluated on general queries (13.7\% hard: conversational). Learning rate η=1.0 for Corralling. Deterministic evaluation (seed=42).
\end{tablenotes}
\end{table}

\subsection{The Cost of Mismatch: Quantifying Negative Transfer}

To quantify the challenge of negative transfer, we computed the semantic alignment between our warmup prior (trained on RouteLLM) and the target production distribution (LMSYS Arena dev set). The resulting score of \textbf{0.48 indicates significant domain mismatch}. Under these conditions, a standard prior-driven router (Warmup Only) suffers a 2.9× regret penalty (126 vs 43 optimal). 
...
The -57.1\% improvement metric quantifies Corralling's \textbf{robustness value}:
