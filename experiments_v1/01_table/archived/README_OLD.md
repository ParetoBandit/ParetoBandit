...
**⚠️ IMPORTANT LIMITATION**: Categories are validated using LLM annotators, not human ground truth:
- **Models**: GPT-4o-mini, Claude-3-Haiku, Llama-3.3-70b
- **Sample**: 100 prompts
- **Inter-LLM agreement**: Fleiss' κ = 0.75 (substantial agreement among LLMs)
- **Heuristic accuracy vs LLM consensus**: 49% (see `validation_results_100.json`)
- **Status**: LLM-based validation only; human validation recommended but not performed
- **Impact**: Categories serve descriptive purposes; main experimental findings do not depend on category accuracy
