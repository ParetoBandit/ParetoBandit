...
\textbf{1. The "Trap" is Rare but Costly.}
While the ``High PC1'' cluster (where Mixtral wins) shrinks to 5.9\%, it remains the most dangerous segment. A static router that sends these complex tasks to GPT-4-Turbo (based on a ``Hard = Expensive'' heuristic) actively harms the user experience on the most technical queries, creating a ``Quality Hole'' in the service.

\textbf{2. The "Nuance" is Expensive.}
The growth of the ``Low PC1'' cluster (to 94.1\%) means that production traffic is dominated by tasks where GPT-4-Turbo has a $+0.13$ advantage. This validates the need for a \textbf{Cost-Aware} objective. A naive ``minimize cost'' router would switch 94\% of traffic to Mixtral, sacrificing significant quality. \texttt{banditGPT}'s ability to find the \emph{sub-manifold} within this 94\% where Mixtral is ``good enough'' is the only path to the reported 27\% cost savings.

\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{experiments_v1/01_figure_1M/results/figure1_lmsys_1M_pca.png}
\caption{\textbf{Semantic Structure at Scale.} PCA projection of 594,199 LMSYS Chat-1M prompts reveals overwhelming dominance of routine tasks (Low PC1: 94.1\%, blue). The $PC1=0.3$ decision boundary (dashed line) remains stable despite 317$\times$ scale increase. High PC1 cluster (red, 5.9\%) represents specialized tasks where Mixtral outperforms GPT-4-Turbo. This distribution shift from holdout (82.4\%/17.6\%) to production (94.1\%/5.9\%) proves we cannot simply ``route easy to cheap'': the Performance Paradox makes routing fundamentally context-dependent.}
\label{fig:scale_validation}
\end{figure}

\textbf{Conclusion.}
The 1M scale validation proves that we cannot simply ``route easy tasks to cheap models'' (because the ``easy'' tasks actually benefit from GPT-4-Turbo). Instead, we must \textbf{surgically offload the ``Paradox'' tasks} (High PC1) to the cheap model to fund the usage of GPT-4-Turbo on the routine tasks that necessitate it.
