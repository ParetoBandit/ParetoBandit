...
\paragraph{Contextual Bandits and Warm-Start Methods.}
LinUCB~\cite{li2010contextual} and its variants~\cite{abbasi2011improved} provide principled online learning but can suffer substantial cold-start regret in large action spaces. Warm-start bandit approaches~\cite{bietti2021contextual} leverage offline datasets to improve initialization, but assume (1)~large offline datasets ($>$10K samples) and (2)~i.i.d. deployment distribution matching calibration data. \textbf{Our contribution:} Through rigorous 5-fold cross-validation, we demonstrate that these assumptions fail in LLM routing: warm-start on <1K prompts exhibits consistent negative transfer (+32\% regret, 100\% fold consistency). We identify two failure modes (Herd Suppression, Overfitting) and validate metadata-guided cold start as superior: public metadata (cost, context, benchmarks) handles constraints; online learning handles quality preferences. This enables zero-calibration deployment without the negative transfer risks of offline-trained covariance matrices.
