#!/usr/bin/env python3
"""
Compute a Composite Factual QA Score (CFS) per model.

This script reads from the models_cache.json and computes a statistically
principled factual QA score using Bayesian latent factor modeling.

Default benchmarks:
- mmlu_pro: MMLU-Pro massive multitask language understanding
- gpqa: Graduate-level science QA requiring domain expertise
- arena_rank_expert: LMArena expert category ranking (user preferences)

Custom benchmarks can be specified via --benchmarks or --config.

Usage:
    # Use defaults
    python scripts/compute_factual_qa_score.py
    python scripts/compute_factual_qa_score.py --bayesian
    
    # Custom benchmarks
    python scripts/compute_factual_qa_score.py --benchmarks mmlu_pro:1:0.5 gpqa:1:0.5
    
    # From config file
    python scripts/compute_factual_qa_score.py --config my_benchmarks.json
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute Composite Factual QA Score (CFS).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use default benchmarks
  python compute_factual_qa_score.py --bayesian
  
  # Custom benchmarks (format: field:scale:weight)
  python compute_factual_qa_score.py --benchmarks mmlu_pro:1:0.5 gpqa:1:0.5
  
  # Load from config file
  python compute_factual_qa_score.py --config factual_qa_config.json
  
  # Save default config
  python compute_factual_qa_score.py --save-config factual_qa_config.json
"""
    )
    ...
    parser.add_argument(
        ...
        help="Override the score prefix (default: from suite config, e.g., 'cfs')."
    )
...
def main():
    ...
    if args.list_benchmarks:
        print("Default Factual QA Benchmarks (CFS):")
        print_benchmark_suite(FACTUAL_QA_BENCHMARKS)
        return
    ...
    suite = parse_benchmark_args(
        ...
        default_suite=FACTUAL_QA_BENCHMARKS,
    )
