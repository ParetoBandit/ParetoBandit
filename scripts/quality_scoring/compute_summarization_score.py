#!/usr/bin/env python3
"""
Compute a Composite Summarization Score (CSS) per model.

This script reads from the models_cache.json and computes a statistically
principled summarization score using Bayesian latent factor modeling.

Default benchmarks:
- summedits_score: SummEdits benchmark measuring summary quality across domains
- hallucination_rate: Hallucination rate (inverted - lower is better)
- arena_rank_longer: LMArena longer query ranking (user preferences)

Custom benchmarks can be specified via --benchmarks or --config.

Usage:
    # Use defaults
    python scripts/compute_summarization_score.py
    python scripts/compute_summarization_score.py --bayesian
    
    # Custom benchmarks
    python scripts/compute_summarization_score.py --benchmarks summedits_score:1:0.5 hallucination_rate:1:0.5
    
    # From config file
    python scripts/compute_summarization_score.py --config my_benchmarks.json
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute Composite Summarization Score (CSS).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use default benchmarks
  python compute_summarization_score.py --bayesian
  
  # Custom benchmarks (format: field:scale:weight)
  python compute_summarization_score.py --benchmarks summedits_score:1:0.5 hallucination_rate:1:0.5
  
  # Load from config file
  python compute_summarization_score.py --config summarization_config.json
  
  # Save default config
  python compute_summarization_score.py --save-config summarization_config.json
"""
    )
    ...
    parser.add_argument(
        "--min-benchmarks", type=int, default=2,
    ...
    parser.add_argument(
        ...
        help="Override the score prefix (default: from suite config, e.g., 'css')."
    )
...
def main():
    ...
    if args.list_benchmarks:
        print("Default Summarization Benchmarks (CSS):")
        print_benchmark_suite(SUMMARIZATION_BENCHMARKS)
        return
    ...
    suite = parse_benchmark_args(
        ...
        default_suite=SUMMARIZATION_BENCHMARKS,
    )
