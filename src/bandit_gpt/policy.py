...
# ---------------------------------------------------------------------------
# Sliding Window LinUCB (SW-UCB baseline for non-stationary experiments)
# ---------------------------------------------------------------------------


class SlidingWindowLinUCBPolicy:
    """Sliding Window LinUCB: retains only the most recent *W* observations.

    Non-stationarity is handled by discarding observations older than *W*
    steps, following the Sliding-Window UCB family of Garivier and
    Moulines (2011).  Unlike geometric discounting (D-UCB /
    :class:`DisjointLinUCBPolicy` with ``gamma < 1``), this gives
    **equal** weight to all observations within the window and **zero**
    weight to observations outside it.

    On each :meth:`update`, the sufficient statistics ``A`` and ``b`` for
    the updated arm are rebuilt from all observations in the current
    window.  This costs O(W_a · d²) per update (where W_a is the number
    of observations for arm *a* in the window) but guarantees numerical
    stability — no floating-point drift from incremental add/subtract.
    ...
    init_lambda : float
        Ridge regularization (``A_0 = init_lambda · I``).
    window_size : int
        Maximum number of recent observations retained across all arms.
    seed : int | None
        RNG seed for tie-breaking in :meth:`select_arm`.

    References
    ----------
    Garivier, A. & Moulines, E. (2011). On Upper-Confidence Bound
    Policies for Switching Bandit Problems. *ALT 2011*.
    """
    ...
    def __init__(
        ...
        self.dim = int(dim)
        self.alpha = float(alpha)
        self.gamma: float = 1.0  # API compat; SW-UCB uses no per-step decay
        self.init_lambda = float(init_lambda)
    ...
    def _rebuild_arm(self, arm: str) -> None:
        """Recompute A, b, A_inv, theta for *arm* from buffer contents."""
        A = np.eye(self.dim) * self.init_lambda
        b = np.zeros(self.dim, dtype=np.float64)
        for buf_arm, x, reward, weight in self._buffer:
            if buf_arm == arm:
                A += weight * np.outer(x, x)
                b += weight * reward * x
        self.A[arm] = A
        self.b[arm] = b
        self.A_inv[arm] = safe_inv(A)
        self.theta[arm] = self.A_inv[arm] @ b

    # ------------------------------------------------------------------
    # Selection (identical to DisjointLinUCBPolicy, minus staleness)
    # ------------------------------------------------------------------
    ...
    def select_arm(
        ...
        Uses the cached ``theta`` and ``A_inv`` computed during the last
        :meth:`update`.  No staleness inflation is applied — the sliding
        window handles non-stationarity.
    ...
    def mark_selected(self, model: str) -> None:
        """Record selection and advance the global clock."""
        with self._lock:
            self.t += 1
            self.last_played[model] = self.t
    ...
    def update(
        ...
        """Add an observation and rebuild affected arm statistics.

        If the buffer exceeds ``window_size``, the oldest observation is
        evicted.  The arm(s) whose statistics changed (the updated arm,
        and the evicted arm if different) are rebuilt from the current
        buffer contents.
        ...
        advance_time : bool
            Whether to increment ``self.t`` (default ``True``).
        """
        ...
        arms_to_rebuild = {model}
        if evicted_arm is not None and evicted_arm != model:
            arms_to_rebuild.add(evicted_arm)

        with self._lock:
            for arm in arms_to_rebuild:
    ...
    def reset_to_tabula_rasa(self) -> None:
        """Clear the observation buffer and reset all arm statistics."""
        with self._lock:
            self._buffer.clear()
            for m in self.models:
                self.A[m] = np.eye(self.dim) * self.init_lambda
                self.b[m] = np.zeros(self.dim, dtype=np.float64)
                self.A_inv[m] = safe_inv(self.A[m])
                self.theta[m] = self.A_inv[m] @ self.b[m]
                self.last_update[m] = 0
                self.last_played[m] = 0
            self.t = 0

    # ------------------------------------------------------------------
    # Inference helpers (API compat with DisjointLinUCBPolicy)
    # ------------------------------------------------------------------
    ...
    def get_ucb_variance(self, model: str, x: np.ndarray) -> float:
        """UCB variance term ``x^T A_inv x`` (no staleness inflation)."""
        return float(x.dot(self.A_inv[model]).dot(x))

    def refresh_inverse_cache(self) -> None:
        """Rebuild all ``A_inv`` and ``theta`` caches from current ``A``."""
        with self._lock:
            for m in self.models:
                self.A_inv[m] = safe_inv(self.A[m])
                self.theta[m] = self.A_inv[m] @ self.b[m]
    ...
    def _check_numerical_stability(
        ...
        """No-op: SW-UCB rebuilds from scratch each update, so no drift."""

    def _apply_staleness_inflation(self, *args: Any, **kwargs: Any) -> None:
        """No-op: SW-UCB handles non-stationarity via window, not inflation."""
