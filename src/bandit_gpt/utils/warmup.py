...
**Motivation:** An identity covariance provides no structural confidence; the
bandit may waste exploration on feature combinations that rarely occur in
practice (e.g., "math without LaTeX").

**Approach:** Generate synthetic archetypes that capture feature correlations:
...
def procedural_warmup(router: BanditRouter, n_samples: int = 50):
    """
    [DEPRECATED - DISABLED] Procedural warmup disabled due to feature dimension mismatch.
    
    **Why Disabled:**
    This function was designed for an old feature structure:
    ...
    It would set indices 17-22 thinking they're "anchors", but those are actually
    PCA components 17-22. This corrupted the feature space with meaningless values.
    
    **Impact of Disabling:**
    - No feature space corruption ✓
    - Router starts with identity matrix + quality priors (standard LinUCB)
    - Cold start may take ~20-50 requests to converge (vs ~15 with working warmup)
    - This is acceptable - correctness > speed
    
    **Solution:** Use warmup joblib files generated by scripts/generate_warmup.py
    for proper initialization with real prompt distributions.
    
    Args:
        router: BanditRouter instance to warm up
        n_samples: Number of synthetic samples to generate [IGNORED - warmup disabled]
    """
    logger.warning(
        ...
        "   \n"
        "   Solution: Use warmup joblib files for proper initialization:\n"
        "     router = BanditRouter.create(priors='artifacts/priors_warmup.joblib')\n"
        "   \n"
        "   See: scripts/generate_warmup.py"
    )
    return
