...
class BanditRouter:
    ...
    def create(
        ...
        priors_path: Optional[Path | str] = None,  # Custom path to priors file
        ...
        Args:
            ...
            priors_path: Custom path to priors metadata file (.npz). 
                         If None, defaults to src/bandit_gpt/priors/priors_meta_pca.npz
        ...
        if priors == "hle":
            ...
            if priors_path:
                meta_path = Path(priors_path)
            else:
                meta_path = base_dir / "priors" / "priors_meta_pca.npz"
            
            if not meta_path.exists():
                 logger.warning(f"No priors metadata found at {meta_path}. Falling back to cold start.")
