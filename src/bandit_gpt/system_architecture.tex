...
This vector $\boldsymbol{\theta}_{\text{neighbor}} \in \mathbb{R}^d$ represents the neighbor's learned utility function over the $d$-dimensional semantic space (typically $d=33$ after PCA compression: 32 PCA components + 1 bias).
...
\paragraph{Memory Footprint:} Each model requires $O(d^2)$ memory for $\mathbf{A}_m$ (d=33 → 8.7 KB per model). For 100 models, total overhead is 870 KB.
