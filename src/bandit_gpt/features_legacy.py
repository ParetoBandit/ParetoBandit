"""
⚠️ DEPRECATED: This file is deprecated and will be removed in v2.0.

**WHY DEPRECATED:**
This implementation uses only 1-dimensional features (length_penalty_log),
making the router unable to distinguish semantic differences between prompts.

Example: "Write a poem" vs "Solve calculus" → treated identically if same length

**MIGRATION TO feature_service.py:**
```python
# OLD (deprecated)
from bandit_gpt.features import FeatureExtractor
extractor = FeatureExtractor()
features = extractor.extract_features(text)  # (1,) array

# NEW (recommended)  
from bandit_gpt import FeatureService
service = FeatureService()
features = service.extract_features(text)  # (24,) array with semantics
```

---

Feature Engineering (Regex/Transforms) - LEGACY VERSION
