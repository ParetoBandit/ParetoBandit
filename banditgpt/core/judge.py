...
class PriorLoadError(RuntimeError):
    """Raised when priors cannot be loaded."""


class PriorSaveError(RuntimeError):
    """Raised when priors cannot be saved."""
...
class PriorManager:
    ...
    def load(self) -> Optional[Dict[str, Any]]:
        ...
        # Validate bundled priors once (cached manifest)
        if path.resolve().is_relative_to(self.BUNDLED_PRIORS_PATH.parent.resolve()):
            try:
                manifest = load_priors_manifest(cache=True)
                verify_bundled_prior(path, manifest=manifest)
            except Exception as exc:  # pragma: no cover - integrity failures
                raise PriorLoadError(f"Bundled priors failed integrity check: {exc}") from exc

        priors = self._load_npz(path)
        logger.debug(
            "Loaded priors",
            extra={
                "path": str(path),
                "source": self.config.source,
                "models": len(priors.get("model_ids", [])),
                "dim": priors.get("dim"),
            },
        )
        return priors
    ...
    def _load_npz(self, path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PriorLoadError(
                f"Priors file {path} cannot be loaded without pickle; "
                "ensure it is a standard NPZ generated by BanditGPT."
            ) from exc
        except OSError as exc:
            raise PriorLoadError(f"Failed to load priors from {path}: {exc}") from exc
        result: Dict[str, Any] = {}
        ...
        # Load metadata (support both old dict format and new array format)
        if "meta_dim" in data:
            result["dim"] = int(data["meta_dim"][0])
        if "meta_alpha" in data:
            result["alpha"] = float(data["meta_alpha"][0])
        # Fallback for old format (requires allow_pickle=True, but we try anyway)
        if "meta" in data and "dim" not in result:
            try:
                meta = data["meta"].item() if hasattr(data["meta"], "item") else dict(data["meta"])
                result.update(meta)
            except Exception:
                pass  # Old format not loadable without pickle
    ...
    def save(self, priors: Dict[str, Any], *, path: Optional[Path] = None) -> Path:
        ...
        try:
            ...
            raise PriorSaveError(
                f"Cannot create priors directory {out_path.parent}: {exc}. "
                "Set BANDITGPT_USER_PRIORS_DIR or pass an explicit writable path."
            ) from exc
        ...
        # Save
        # Use fixed-width string dtype to avoid object arrays (which require pickle)
        model_ids_arr = np.array(model_ids, dtype="U256")  # Unicode strings up to 256 chars
        # Store meta as individual arrays to avoid object arrays
        np.savez_compressed(
            out_path,
            A_shared=np.asarray(A_shared, dtype=np.float16) if A_shared is not None else np.eye(dim, dtype=np.float16),
            b_vectors=b_matrix,
            model_ids=model_ids_arr,
            meta_dim=np.array([int(dim)], dtype=np.int32),
            meta_alpha=np.array([float(alpha)], dtype=np.float32),
        )
