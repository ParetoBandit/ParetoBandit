...
A context-free router that ignores prompt content must commit to a single, fixed traffic distribution (e.g., 70\% budget, 25\% mid-cost, 5\% frontier) regardless of prompt difficulty.
...
We enforce this constraint via an online Primal-Dual Lagrangian relaxation, adapting the dual-variable mechanism of Agrawal and Devanur~\cite{agrawal2014bandits} from a finite-horizon cumulative knapsack to a \emph{rate constraint} suited for open-ended deployment. The system is given a target average budget $B$ (e.g., ``average \$0.005 per req''). The policy $\pi$ must maximize cumulative reward subject to the constraint that the long-run average cost does not exceed $B$:
\begin{equation}
...
\paragraph{Rate constraint vs.\ cumulative knapsack.}
The classical Contextual Bandit with Knapsacks (CBwK) formulation~\cite{agrawal2014bandits} assumes a cumulative budget $\sum_{t=1}^T c_t \leq B_{\mathrm{total}}$ over a known, finite horizon $T$, with dual updates that account for the remaining budget fraction.
Production LLM routing is fundamentally open-ended: request volume is unknown and variable, and the operator's natural specification is a cost \emph{rate} (dollars per request), not a finite pool.
Our formulation inherits the Primal-Dual decomposition---splitting the constrained routing problem into unconstrained bandit subproblems coupled through a dual variable $\lambda_t$---but the dual update targets the running average cost rather than a depleting budget, making the system horizon-free: it self-regulates at any traffic volume without requiring $T$ as input.
Three independent departures from the classical CBwK analysis void the formal $O(\sqrt{T})$ regret and budget-violation bounds of the original finite-horizon proof:
\begin{enumerate}[nosep,leftmargin=*]
  \item \emph{Rate constraint vs.\ cumulative knapsack.}  The CBwK proof requires a depleting cumulative budget with dual updates that reference the remaining budget fraction.  The rate-constraint formulation has no finite horizon, so the convergence argument does not apply.
  \item \emph{EMA smoothing.}  The CBwK dual update requires the raw per-step constraint violation $g_t = c_t - B$ as a valid subgradient of the per-round Lagrangian.  The EMA $\bar{c}_t$ is a biased, lagged estimator of the current cost, voiding the subgradient condition (see Balseiro and Gur~\cite{balseiro2019learning} for the closest theoretical treatment of smoothed dual updates in online pacing).
  \item \emph{Static penalty $\lambda_s$.}  The user-supplied cost penalty $\lambda_s$ is a fixed offset outside the Lagrangian framework; it does not adapt to realized costs and shifts the effective constraint threshold.
\end{enumerate}
We do not claim formal regret or budget-violation bounds for the combined system.  Convergence and budget compliance are validated empirically across five experiment families (Sections~\ref{sec:results}--\ref{sec:discussion}).
This design choice carries a tradeoff.
Because the rate constraint tracks an EMA-smoothed average rather than a cumulative total, a sustained shift toward expensive requests can temporarily push realized spend above $B$ until the dual variable corrects---a lag bounded by the EMA half-life (${\sim}14$ observations at $\alpha_{\mathrm{ema}}{=}0.05$).
Conversely, a finite-horizon knapsack that underestimates $T$ exhausts its budget early, forcing service degradation for the entire remaining tail of the horizon.
We argue in Section~\ref{sec:discussion} that the rate constraint's bounded transient overshoot is strictly preferable to the knapsack's catastrophic quality cliff under traffic variability.
