...
### The Confidence Boost (λ_boost)

Even with expert priors, raw values may be "weak" (only 2,500 updates). The boost multiplier:

```python
A_boosted = A * 50  # "Pretend we saw 50x more data"
b_boosted = b * 50
```

Reduces uncertainty: `sqrt(x' A⁻¹ x)` → smaller → less exploration → immediate exploitation.
