...
($n_{\text{eff}}{=}10$, $\alpha{=}0.1$, disjoint LinUCB,
$\gamma{=}0.997$; selected via joint epsilon-constraint tuning,
...
at the tightest pacer target (\$0.00008/req), the pacer
reaches mean reward $0.811 \pm 0.0005$, while the nearest static
point ($\lambda_s{=}1.0$ at \$0.00029/req---$3.6\times$ higher
spend) achieves only $0.802 \pm 0.0003$: a $+0.9$\,pp quality
advantage at less than a third of the cost.
At moderate budgets (\$0.0007/req), the pacer achieves
$0.880 \pm 0.0014$ reward vs.\ the nearest static point
($\lambda_s{=}0.5$: $0.861 \pm 0.0008$ at \$0.0014).
...
matches the pacer:
$\lambda_s{=}0$ achieves the quality ceiling ($0.923$) because it
imposes zero cost constraint.
The pacer at its loosest target ($B{=}\$0.015$) reaches $0.923$
at \$0.0082---the dual variable correctly decays to zero and the
...
For the six tightest targets, budget utilisation ranges from
$0.98\times$ to $1.07\times$---effectively perfect compliance.
The loosest target ($B{=}\$0.015$) shows
under-utilisation ($0.54\times$) because the
...
oscillates in a bounded band, actively steering a
...
$\bar{c}_t$ to relax.  The bounded oscillation is characteristic
of a primal-dual controller in steady state orbiting the constraint
boundary.
At the loose-binding target ($B{=}\$0.0053$), $\lambda_t$
applies light pressure and the router achieves
$0.98\times$ utilisation with negligible quality loss.
...
\begin{enumerate}[leftmargin=*,noitemsep]
  ...
  \item \textbf{Cost at scale.}
    ...
    Moving from the unconstrained policy (\$0.0082/req, quality~0.923)
    to the pacer at $B{=}\$0.0019$/req saves
    $(\$0.0082 - \$0.0019) \times 100{,}000 = \$630$/day
    (\textbf{\$230{,}000/year})
    while maintaining quality~$0.905 \pm 0.0009$---a 1.9\% quality
    reduction for a 77\% cost reduction.

  \item \textbf{Budget compliance as an SLA.}
    The $0.98$--$1.07\times$ utilisation range at binding targets
