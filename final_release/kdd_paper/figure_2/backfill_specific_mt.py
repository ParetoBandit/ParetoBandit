#!/usr/bin/env python3
"""
Multi-threaded backfill for a SPECIFIC model.
Usage: python3 backfill_specific_mt.py [model_id]
"""
...
class SpecificBackfiller:
    def __init__(self, target_model, api_key, num_workers=10):
        self.target_model = target_model
        self.api_key = api_key
        self.base_url = "https://openrouter.ai/api/v1"
        self.num_workers = num_workers
        self.judge = "anthropic/claude-sonnet-4.5" # Explicitly requested
    ...
    def process_pair(self, prompt_text, cluster_id, dataset):
        try:
            ...
            payload = {
                ...
                "temperature": 0.7,
                "max_tokens": 5000, # Restored to 5000 for standard models
                "stream": False
            }
...
**Rating:**"""
    ...
    def run(self):
        print(f"Starting backfill for {self.target_model}")
        print(f"Workers: {self.num_workers}, Max Tokens: 5000")
        ...
        for dataset in ['train', 'test']:
            with open(DATA_DIR / f'{dataset}_rewards.jsonl') as f:
                for line in f:
                    try:
                        r = json.loads(line)
                        if r.get('ok') and r['model_id'] == self.target_model and r['cluster_id'] in [36, 80]:
...
if __name__ == "__main__":
    ...
    backfiller = SpecificBackfiller(model, API_KEY, num_workers=10)
    backfiller.run()
