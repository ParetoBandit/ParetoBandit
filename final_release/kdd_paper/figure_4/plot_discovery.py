...
def plot_discovery():
    ...
    rects1 = ax.bar(x - width/2, priors, width, label='Prior Confidence (HLE)', color='#bdc3c7', alpha=0.8)
    rects2 = ax.bar(x + width/2, posteriors, width, label='Posterior Confidence (Learned)', color='#3498db', alpha=0.9)
    
    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Specialist Confidence (||\u03b8||)', fontsize=12)
    ax.set_title('Figure 4: Specialist Discovery (Beyond the Teacher)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
