...
from generate_cost_based_priors import (
    analyze_cost_latency_performance,
    generate_cost_latency_prior
)
...
def load_models_with_cluster_data(models_path: Path) -> List[Dict]:
    ...
    # Filter to models with required data
    valid_models = [
        m for m in models
        if all(k in m for k in ['cluster_success_rates', 'price_1m_blended', 
                                'time_to_first_token_seconds', 'openrouter_id'])
...
def evaluate_predictions(holdout_models: List[Dict],
    ...
    for model in holdout_models:
        ...
        # Generate prediction
        prediction = generate_cost_latency_prior(
            cost=cost,
            latency=latency,
...
def main():
    ...
    print("="*70)
    print("HOLDOUT VALIDATION: Cost+Latency Prior Generator")
    print("="*70)
    ...
    for i, model in enumerate(holdout_models):
        ...
        print(f"     Cost: ${model['price_1m_blended']:.2f}, Latency: {model['time_to_first_token_seconds']:.2f}s")
    ...
    print("\n" + "="*70)
    print("Training cost-latency grid on 40 models...")
    print("="*70)
    grid_stats = analyze_cost_latency_performance(temp_path)
    ...
    for pred in results['predictions']:
        ...
        print(f"     Tier: {pred['cost_tier']}/{pred['latency_tier']} (confidence: {pred['confidence']:.2f})")
