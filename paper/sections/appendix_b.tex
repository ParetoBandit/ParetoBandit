...
\textbf{Important caveat:} The results in this appendix are from a \emph{synthetic simulation} (a single model pair with a simulated new model release at $t{=}300$). Controlled ablation experiments on real multi-model data (Section~\ref{sec:transfer_null}) show that semantic transfer provides no statistically significant improvement over tabula rasa initialization. The discrepancy likely arises because the synthetic setup features high reward dispersion and a perfectly matched semantic neighbor---conditions that may not hold in real multi-model portfolios where the new model accounts for $<1/K$ of routing decisions.

We include this appendix for completeness as a sensitivity analysis on the key hyperparameter controlling Latent Semantic Transfer: the \emph{effective prior sample size} ($n_{\text{eff}}$).
...
In this synthetic setting, performance is stable across a $20\times$ range of $n_{\text{eff}}$ (1.0 to 20.0). However, as noted in Section~\ref{sec:transfer_null}, controlled ablation on real multi-model data shows that this improvement does not transfer to production-like conditions. The synthetic setup represents a best-case scenario (perfectly matched neighbor, high reward dispersion, $K{=}2$) that does not hold at larger portfolio sizes. We retain this appendix as a theoretical analysis of the $n_{\text{eff}}$ parameter's Bayesian properties, not as evidence for the practical effectiveness of semantic transfer.
