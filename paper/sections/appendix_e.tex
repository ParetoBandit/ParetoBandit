...
We perform comprehensive validation to ensure our model preference heterogeneity findings (Figure~\ref{fig:alignment_tax}) are statistically robust and not artifacts of methodological choices. All analyses in this section use the held-out test set exclusively ($N=750$).
...
The domain-adapted PCA concentrates routing-relevant variation more efficiently into PC1, yielding a 4.6$\times$ larger effect size. The generic PCA confirms the underlying structure exists independently, validating that the domain-adapted PCA is an effective (not circular) feature extractor. Both PCAs are unsupervised---neither uses reward labels during training.
...
\begin{enumerate}[leftmargin=*,noitemsep,topsep=0pt]
    \item \textbf{Statistical rigor:} $p < 0.0001$, large effect ($d = 1.53$), non-overlapping CIs
    \item \textbf{Threshold justification:} Purely unsupervised (silhouette-optimal, no reward peeking)
    \item \textbf{PCA robustness:} Effect persists with generic PCA ($d = 0.33$), confirming independence
    \item \textbf{High-D caveat:} Weak structure in 384D (silhouette $= 0.057$); PC1 isolates a moderate signal ($\rho^2 = 0.156$)
    \item \textbf{Data quality:} Zero duplicates; High PC1 is a homogeneous cluster (diversity $= 0.355$)
