...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Corralling dominates via complementary schedules.} The meta-learner exploits two experts with complementary alpha schedules, achieving 29--39\% lower regret than either expert alone when priors are useful ($\alpha \leq 0.5$). Under adversarial priors, degradation is detectable at runtime and practitioners can fall back to tabula rasa. Post-failure traffic is redistributed contextually across the healthy portfolio rather than concentrating on a single fallback, and banditGPT matches EMA tracker performance at $K{=}10$.
