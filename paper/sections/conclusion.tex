...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Corralling helps when priors are moderately reliable, and its failure modes are measurable.} The meta-learner combines complementary exploration schedules and reduces regret by 30--40\% relative to either expert alone when priors are useful ($\alpha \leq 0.5$). Under strongly misleading priors, Corralling can degrade; we show this regime is detectable in our stress tests and can be handled with a simple tabula rasa fallback (Section~\ref{sec:prior_degradation}).
