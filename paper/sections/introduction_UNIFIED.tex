...
Distribution shift between training and deployment data is significant (PSI=$0.225$, 95\% CI: $[0.194, 0.285]$; Figure~\ref{fig:distribution_shift}). Under this moderate shift, warmup priors remain beneficial (28\% lower regret than cold-start; Table~\ref{tab:performance_gap}), but more severe domain mismatch---inevitable in real deployments with diverse user populations---could degrade or invert prior quality. Static routers like RouteLLM~\cite{ong2024routellm} and FrugalGPT~\cite{chen2023frugalgpt} have no mechanism to detect or adapt to such shifts.
