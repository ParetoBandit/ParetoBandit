...
\begin{enumerate}
    \item \textbf{Model Preference Heterogeneity (Figure~\ref{fig:lmsys_holdout_structure}):} We demonstrate that model preference varies by prompt and the router's features predict this variation. This establishes that routing is \emph{learnable} through contextual features.
...
Having established empirical motivation---model preference heterogeneity exists (Figure~\ref{fig:lmsys_holdout_structure}) but distribution shift is substantial (Figure~\ref{fig:distribution_shift})---we now present our integrated system design. Our approach combines three mechanisms, each addressing one of the challenges identified in Section~\ref{sec:intro}:
...
\begin{itemize}
    ...
    \item Figures~\ref{fig:lmsys_holdout_structure}--\ref{fig:distribution_shift}: Empirical foundations
\end{itemize}
