...
The three conditions produce clearly separated cumulative regret trajectories across 40 seeds (shown in Figure~\ref{fig:cumulative_regret}):
...
\caption{Cumulative Regret under Reward Drift. BanditGPT ($\gamma=0.997$) adapts rapidly to quality degradation, unlike naive bandits or frozen policies.}
...
Through a sensitivity heatmap across budget targets and varying drift magnitudes (Figure~\ref{fig:hparam_sweep}), we confirm that our jointly-tuned $\gamma=0.997$ structurally balances stationary performance with agility. It forgets stale evidence fast enough to track a shifting optimum while retaining enough recent observations (and discounted priors) for confident, stable exploitation in stationary phases.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/hparam_sweep_heatmap.pdf}
\caption{Hyperparameter Sensitivity Heatmap. BanditGPT's performance is robust across various budget targets and drift magnitudes without requiring environment-specific retuning.}
