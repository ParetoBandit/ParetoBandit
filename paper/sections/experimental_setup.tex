...
To ensure robust and reproducible evaluation, we employ an automated LLM-as-a-judge paradigm. However, relying on a single judge for $K>2$ routing is problematic: binary win/loss outcomes often collapse into a tie between the top two models for a large percentage of prompts, reducing the problem to a binary choice. To address this, we use a \emph{continuous multi-judge ensemble} that outputs a scalar quality score $r_t \in [0, 1]$ for each prompt-model pair. This continuous score is a weighted average of three sub-scores (e.g., helpfulness, accuracy, and tone) evaluated across a panel of independent judges, providing the granular quality distinctions necessary for multi-arm routing.
