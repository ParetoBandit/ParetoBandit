...
- **Hallucination Rate**: 83 models (100% coverage) from multiple sources:
  - Vectara Hallucination Leaderboard (69 models, 83%) - document summarization evaluation
  - Kaggle SimpleQA (3 models) - short-form factual Q&A
  - Voronoi 2025 (11 models) - reasoning model evaluation
- **Factual Consistency Rate**: Derived as 100 - hallucination_rate
- **Answer Rate**: Model compliance/refusal behavior (from Vectara)
- **Arena ELO**: Human preference ground truth (31 models, 37%)

### Trust Score Fields in `models_cache.json`
| Field | Description |
|-------|-------------|
| `hallucination_rate` | Percentage of responses with factual errors |
| `factual_consistency_rate` | 100 - hallucination_rate |
| `hallucination_answer_rate` | % of prompts model responded to |
| `hallucination_source` | Data provenance (vectara_leaderboard, SimpleQA, etc.) |
| `hallucination_source_url` | URL for verification |
| `vectara_model_name` | Original identifier on Vectara leaderboard |
