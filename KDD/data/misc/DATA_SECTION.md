...
### 3.4.1 Hallucination and Trust Metrics

We collect hallucination and factual consistency scores from multiple sources to ensure complete coverage of our model suite. Trust metrics are critical for production deployments where factual accuracy impacts user safety and system reliability.

**Primary Source: Vectara Hallucination Leaderboard.** The majority of our hallucination data (69/83 models, 83%) comes from the Vectara Hallucination Leaderboard \cite{vectara2024hallucination}, which evaluates factual consistency using a document summarization protocol:

- **Methodology**: Models are asked to summarize source documents. Summaries are evaluated for factual consistency against the source using the HHEM (Hughes Hallucination Evaluation Model) classifier, validated against expert human annotations.
- **Test Set**: 1,000 document-summary pairs across diverse domains
- **Evaluation**: Automated classification with human-annotated ground truth validation
- **GitHub Repository**: github.com/vectara/hallucination-leaderboard

The leaderboard provides four complementary metrics:

| Metric | Description | Use in Our System |
|--------|-------------|-------------------|
| **Hallucination Rate** | Percentage of summaries containing factual errors | Primary trust metric |
| **Factual Consistency Rate** | 100 - Hallucination Rate | Direct quality indicator |
| **Answer Rate** | Percentage of prompts the model responded to | Measures compliance/refusal behavior |
| **Avg Summary Length** | Mean word count of generated summaries | Controls for length bias |

We store the raw hallucination rate and derive factual consistency via $S_{\text{factual}} = 100 - h$ for integration into composite quality scores.

**Alternative Sources for Extended Coverage.** For models not present on the Vectara leaderboard (14/83 models, 17%), we obtain hallucination metrics from alternative benchmarks with compatible evaluation protocols:

| Source | N Models | Methodology | Coverage |
|--------|----------|-------------|----------|
| Vectara Leaderboard (current) | 36 | Document summarization with HHEM | Primary |
| Vectara Leaderboard (legacy) | 33 | Older HHEM version for discontinued models | Secondary |
| Kaggle SimpleQA | 3 | Short-form factual Q&A | Fallback |
| Voronoi 2025 | 11 | Reasoning model evaluation | Fallback |

**Rationale for Multi-Source Approach**: Different hallucination benchmarks measure related but distinct aspects of factual reliability. Vectara focuses on summarization fidelity, SimpleQA on factual recall, and Voronoi on reasoning model truthfulness. We prioritize Vectara for consistency but supplement with alternatives to achieve 100% coverage. Source provenance is tracked in `hallucination_source` and `hallucination_source_url` fields for transparency.

**Model Name Matching.** The Vectara leaderboard uses non-standardized model identifiers (e.g., `google/gemini-2.5-flash`). We maintain a curated mapping in `llm_jury/etl/hallucination_leaderboard_client.py` that maps leaderboard names to our canonical model names (e.g., `google/gemini-2.5-flash` → `Gemini 2.5 Flash`). The `vectara_model_name` field in our cache preserves the original leaderboard identifier for verification and reproducibility.

**Data Quality Validation.** We validate hallucination scores through:
1. **Cross-source consistency**: For models with scores from multiple sources, we verify agreement within 5 percentage points
2. **Temporal stability**: Re-fetching Vectara data weekly confirms <1% drift for established models
3. **Outlier review**: Extreme hallucination rates (>50%) are manually verified against model documentation
