...
\begin{itemize}[nosep,leftmargin=*]
  \item \textbf{Fixed Policy under-spends} ($0.45$--$0.53\times$).
    ...
    At tight budget, it routes ${\sim}71\%$ to Llama-8B---the cheapest arm
  ...
  \item \textbf{Naive Bandit is inconsistent across budgets.}
    ...
    overshoots at tight ($1.28\times$) and happens to land near
    target at moderate ($0.96\times$)---but this alignment is
    coincidental, not controlled.
...
post-drop costs are only $0.18\times$ the target, and
$\lambda_t$ drops from $0.09$ to near-zero almost immediately.
At \textbf{tight} budget, $\lambda_t$ decays from $0.41$ in
Phase~1 to ${\sim}0.05$ in Phase~2---slower than moderate or
loose because the margin is smaller.  Post-drop costs
(\$2.85e-4/req) are $0.95\times$ the tight target
(\$3.0e-4), so the cost EMA settles just below target and
$\lambda_t$ stabilizes at a small positive value rather than
reaching zero.
...
${\sim}68\%$---the highest of any constrained condition.
Two mechanisms contribute: the residual Phase~1 $\lambda_t$
(${\sim}0.41$) persists into early Phase~2, penalizing Mistral's
non-zero normalized cost and driving initial Gemini concentration;
as $\lambda_t$ decays to ${\sim}0.05$, the router settles into a
Gemini-heavy but mixed policy, since Gemini is simultaneously
...
loose because its residual $\lambda_t$ (${\sim}0.05$) still
provides mild cost pressure favoring the cheapest arm, whereas
at moderate and loose $\lambda_t$ reaches near-zero and the router
quality-optimizes with a mixed Gemini/Mistral policy.
...
${\sim}0.4\%$ ($0.919$ vs.\ $0.922$), attributable to the residual
$\lambda_t$ that slightly favors cheaper arms over the
quality-optimal mix.
%
...
($\Delta{=}+0.017$ to $+0.054$) confirms that the forgetting
...
\midrule
Tight    & \$3.00e-4 & \$2.99e-4 & \$2.85e-4 & \$2.92e-4 & $0.97\times$ \\
...
\noindent
ParetoBandit achieves Phase~1 costs within 2\% of the target at all
...
tight costs settle at $0.97\times$ target, moderate at $0.75\times$,
and loose at $0.58\times$.
...
\midrule
Tight    & 0.864 & 0.919 & $+0.054$ & $[0.052,\; 0.056]$ \\
...
($\Delta{=}+0.054$), reflecting the shift from a
Llama/Mistral mix to $68\%$ Gemini routing once the price drop
...
constraint transitions from \emph{binding} to \emph{slack} at
the price-change boundary.
In Phase~1, $\lambda_t$ averages $0.41$, enforcing a Llama/Mistral
mix with negligible Gemini selection ($<1\%$).
After the price drop, post-drop costs (\$2.85e-4/req) fall below
the target, so $\lambda_t$ decays to ${\sim}0.05$.
Despite the constraint becoming slack, the tight condition
still achieves the highest Gemini adoption ($68\%$) of any
constrained condition, because the residual $\lambda_t$ provides
mild cost pressure that favors the now-cheapest arm (Gemini,
$\tilde{c}{\approx}0$) over the relatively expensive Mistral
($\tilde{c}{\approx}0.49$).
This smooth, automatic transition---from aggressively binding
Phase~1 to mildly slack Phase~2---demonstrates that the
primal-dual mechanism correctly identifies when the constraint
relaxes and adjusts accordingly, without operator intervention.
...
$0.95\times$ target), and $\lambda_t$ settles at a low but
positive value (${\sim}0.05$).
...
\midrule
\multirow{4}{*}{Tight ($\$3.0{\times}10^{-4}$)}
  & Fixed Policy        & $0.53\times$ & $0.88\times^{\dagger}$ \\
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item The \textbf{Naive Bandit} is inconsistent: it overshoots
    at tight ($1.28\times$), near-tracks at moderate ($0.96\times$),
    and over-spends by $2.9\times$ at loose budget, because no
  ...
  \item \textbf{ParetoBandit} is the only condition that achieves
    precise Phase~1 budget compliance ($0.98$--$1.00\times$ target)
    while automatically exploiting the Gemini price
    drop to improve quality
    ($\Delta_\mathrm{reward}{=}+0.017$ to $+0.054$).
  \item All three budget regimes demonstrate consistent
    ...
    small positive value (${\sim}0.05$) because the margin is narrow.
