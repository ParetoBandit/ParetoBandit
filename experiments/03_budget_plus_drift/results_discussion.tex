...
\paragraph{Phase~2 budget compliance across conditions
(Figure~\ref{fig:exp03_adaptation}d--f).}
The tight-budget regime reveals the sharpest contrast in
Phase~2 budget compliance:

\begin{center}
\small
\begin{tabular}{@{}llrr@{}}
\toprule
\textbf{Budget} & \textbf{Condition} & \textbf{Phase~2 Cost} & \textbf{Ratio} \\
\midrule
\multirow{4}{*}{Tight ($\$2.3 \times 10^{-4}$)}
  & Fixed Policy     & \$2.6e-4 & $1.13\times$ \\
  & Naive Bandit     & \$2.6e-4 & $1.11\times$ \\
  & Recalibrated     & --- & --- \\
  & \textbf{BanditGPT} & \textbf{\$2.4e-4} & $\mathbf{1.02\times}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The Fixed Policy and Naive Bandit overshoot the tight target by
$11$--$13\%$ in Phase~2 because their static cost penalties
cannot adapt to the new cost landscape.
The Recalibrated Bandit re-tunes its $\lambda_s$ at the Phase~2
boundary and can close part of this gap---but its one-shot
recalibration is sensitive to the calibration set and cannot
adjust within Phase~2 as spending conditions evolve.
BanditGPT stays within $2\%$ of target: the pacer's dual
variable continuously adjusts the cost--quality tradeoff to
maintain compliance even as the pricing regime changes.

\emph{Note: the Recalibrated row above uses placeholder values;
update after re-running the experiment.}
