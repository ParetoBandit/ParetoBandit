...
$0.35$ during Phase~1 then drops sharply
to ${\sim}0.01$ within Phase~2:
...
At \textbf{tight} budget, $\lambda_t$ \emph{rises} from $0.46$
in Phase~1 to $0.82$ in Phase~2.
...
${\sim}84\%$---the highest of any constrained condition.
...
${\sim}2\%$ to ${\sim}49\%$, approaching the unconstrained
baseline (${\sim}75\%$ Gemini in Phase~2).
At \textbf{loose} budget, Gemini selection grows from ${\sim}10\%$
to ${\sim}47\%$, also converging toward the unconstrained level.
...
\midrule
Unconstr.\ & ---     & \$1.19e-2 & \$3.14e-4 & \$6.11e-3 & --- \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Tight    & 0.852 & 0.917 & $+0.065$ & $[0.064,\; 0.067]$ \\
Moderate & 0.877 & 0.921 & $+0.044$ & $[0.041,\; 0.048]$ \\
Loose    & 0.904 & 0.921 & $+0.017$ & $[0.015,\; 0.018]$ \\
\bottomrule
\end{tabular}
\end{center}
...
($\Delta{=}+0.065$), reflecting the dramatic shift from a
Llama/Mistral mix to $84\%$ Gemini routing once the price drop
makes Gemini the cheapest arm in the portfolio.
The moderate lift ($\Delta{=}+0.044$) is similar in magnitude;
...
so $\lambda_t$ continues to rise (from $0.46$ to $0.82$).
Paradoxically, this elevated $\lambda_t$ produces the
\emph{highest} Gemini adoption ($84\%$) of any constrained
condition.
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item \textbf{BanditGPT} is the only condition that achieves
    ...
    ($\Delta_\mathrm{reward}{=}+0.017$ to $+0.065$).
