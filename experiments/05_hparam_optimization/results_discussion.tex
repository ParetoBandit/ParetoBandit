...
\caption{%
  ...
  AUC-only optimisation selects $\gamma{=}1.0$ for both
  variants---the maximally stationary configuration that
  maximises AUC but degrades adaptation under reward shift.
...
\multirow{2}{*}{Tabula Rasa}
  & AUC-only     & $1.00$ & ---    & $1.000$ & $0.929$ & --- \\
  & $\epsilon$-constr.\ & $0.10$ & ---    & $0.995$ & $0.928$ & $\mathbf{49.0}$ \\
\bottomrule
\end{tabular}
\end{table}

AUC-only optimisation selects $\gamma{=}1.0$ for \emph{both}
variants---the maximally stationary configuration.
Section~\ref{sec:nonstationarity} shows that $\gamma{=}1.0$
is catastrophic under drift.
...
$0.929 \to 0.928$) for substantially lower Phase-2 regret
($48.4$ and $49.0$).
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item \textbf{Tabula Rasa exploration decreases.}
    AUC-only selects $\alpha{=}1.0$ for Tabula Rasa, while
    $\epsilon$-constraint selects $\alpha{=}0.10$ ($10\times$ lower).
    Without forgetting ($\gamma{=}1.0$), the cold-start variant
    needs aggressive exploration to overcome zero priors;
    with forgetting ($\gamma{=}0.995$), the bounded effective
    sample size naturally re-explores as stale evidence
    is discounted, reducing the need for explicit exploration.
\end{enumerate}
%
\noindent
...
\emph{systematically} selects $\gamma{=}1.0$ for both variants
is itself a cautionary finding: any practitioner deploying a
non-stationary bandit router who tunes via standard
cross-validation will converge on no forgetting at all,
