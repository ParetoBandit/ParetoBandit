...
\multirow{2}{*}{ParetoBandit}
  ...
  & Knee-point   & $\hpParetoBanditAlpha$ & $\hpParetoBanditNeff$ & $\hpParetoBanditGamma$ & $\hpParetoBanditAUC$ & $\hpParetoBanditP2Reward$ \\[3pt]
\multirow{2}{*}{Tabula Rasa}
  & AUC-only     & $\hpTabulaAUCOnlyAlpha$ & --- & $\hpTabulaAUCOnlyGamma$ & $\hpTabulaAUCOnlyAUC$ & --- \\
  & Knee-point   & $\hpTabulaAlpha$ & --- & $\hpTabulaGamma$ & $\hpTabulaAUC$ & $\hpTabulaP2Reward$ \\
\bottomrule
\end{tabular}
\end{table}
...
Section~\ref{sec:nonstationarity} shows is catastrophic under drift.
...
\midrule
Llama-8B             & $\hpCrossArmLlamaP2$   & $\pm\hpCrossArmLlamaStd$ \\
Mistral-Large (tuned on) & $\hpCrossArmMistralP2$ & $\pm\hpCrossArmMistralStd$ \\
Gemini-Pro           & $\hpCrossArmGeminiP2$  & $\pm\hpCrossArmGeminiStd$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item \textbf{Warmup priors remain the structural enabler.}
    ...
    higher P2 reward ($\hpParetoBanditP2Reward$ vs.\
    $\hpTabulaP2Reward$), with substantially lower variance.
