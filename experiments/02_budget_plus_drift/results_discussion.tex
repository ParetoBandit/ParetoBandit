...
The critical panel.  In Phase~1, the Forgetting Bandit's cost trace
sits far above the moderate budget target---already overshooting at
$\bdForgetModPhaseOneRatio$---because geometric forgetting has eroded
the cost penalty's anchoring effect.
The Naive Bandit sits closer to target in Phase~1
($\bdNaiveModPhaseOneRatio$), anchored by its full history.
In Phase~2, both traces drop sharply as cheap Gemini traffic
dominates.
In Phase~3, when normal pricing resumes, both traces spike above
target: the Forgetting Bandit reaches $\bdForgetModPhaseThreeRatio$
and the Naive Bandit $\bdNaiveModPhaseThreeRatio$---neither has a
feedback mechanism to re-tighten cost control.
