...
% Updated: February 21, 2026 — 100-seed results.

% Crossover value from prior degradation experiment (N=100 seeds, w₀=0.7).
\providecommand{\CrossoverWC}{0.55}
% Corralling upside: 18.1 regret saved (34%), downside: 20.0 extra (38%), breakeven: 53%
...
initialization \emph{and} eventual exploitation---achieving 29--39\%
lower regret than either alone.

\emph{Adversarial priors ($\alpha \geq 0.6$):}
\begin{itemize}[nosep]
    \item \textbf{Corralling:} 64--75 regret ($\sigma{\approx}19{-}23$)
    \item \textbf{Warmup-only:} 59--66 regret (monotonic degradation)
    \item \textbf{Tabula Rasa:} ${\sim}52.5$ regret (invariant)
\end{itemize}
...
Warmup-only degrades monotonically (52 to 66) but is \emph{never}
...
\begin{itemize}[nosep]
    \item \textbf{Corralling's upside:} saves 18.1 cumulative regret
          (34\% reduction vs.\ tabula rasa) when $\alpha \leq 0.4$
    \item \textbf{Corralling's downside:} costs 20.0 extra regret
          (38\% increase) when $\alpha \geq 0.6$
\end{itemize}

The breakeven occurs when a practitioner believes there is roughly a
\textbf{53\% chance that priors are useful} ($\alpha \leq 0.4$).  Above
...
to believe priors are adversarial.  The 53\% breakeven threshold is
...
rasa provides a safe, prior-independent baseline at ${\sim}52.5$ regret.
