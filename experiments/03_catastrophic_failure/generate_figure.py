...
``catastrophic_failure_dynamics.pdf/.png``:
  ...
  (a) Gemini-Pro Selection Fraction — explains why constrained conditions
      take a bigger quality hit (budget limits Gemini-Pro adoption).
  (b) Windowed Mean Reward — quality outcome through the crash.
...
RESULTS_FILE = "catastrophic_failure_results.json"
...
def plot_adaptation_dynamics(
    ...
    Panels:
      (a) Gemini-Pro selection fraction — explains the quality gap between
          constrained and unconstrained (budget limits Gemini adoption).
      (b) Windowed mean reward — quality outcome.
      (c) Windowed average cost per request — budget compliance.
...
def main() -> None:
    ...
    for fmt in ("pdf", "png"):
        fig.savefig(
            RESULTS_DIR / f"catastrophic_failure_dynamics.{fmt}",
            bbox_inches="tight",
            dpi=300,
        )
    plt.close(fig)
