...
All baselines follow the same pattern:
  1. Train on dev-train: label = argmax_m reward[m] per prompt (same objective
     as BanditGPT's ``extract_reward``).
  2. Freeze: no online adaptation.
  3. Evaluate on holdout: predict best model for each prompt, look up reward.
...
def run_supervised_baseline(
    ...
    *,
    ...
    out: Dict[str, Any] = {
        ...
        "std_reward": float(np.std(rewards_per_trial)) if effective_trials > 1 else 0.0,
        "std_cost": float(np.std(costs_per_trial)) if effective_trials > 1 else 0.0,
