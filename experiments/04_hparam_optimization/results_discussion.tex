...
Pareto AUC of $\hpBanditGPTTestAUC$ (BanditGPT) and
$\hpTabulaTestAUC$ (Tabula Rasa) versus $\hpFixedTestAUC$ for the
fixed-model envelope---deltas of $\hpBanditGPTTestDelta\%$ and
$\hpTabulaTestDelta\%$, both within one standard deviation of
seed variance ($\pm\hpBanditGPTTestStd$ and
$\pm\hpTabulaTestStd$).
The val-selected hyperparameters thus transfer to test without
statistically significant degradation, consistent with the
$\epsilon$-constraint design which permits up to $0.25\%$ AUC
sacrifice in exchange for adaptation capability.
