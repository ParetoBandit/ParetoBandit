...
```python
from banditgpt.config import RouterConfig, StructuralFeature, AnchorConfig
...
config = RouterConfig(
    ...
    structural_features=[
        StructuralFeature(name="code_block", source="regex_count", pattern="```", transforms=["binarize", "log1p"]),
        StructuralFeature(name="latex", source="regex_count", pattern=r"\$|\\begin", transforms=["binarize", "log1p"]),
    ],
...
```
banditgpt/
├── router.py                 # Main BanditRouter implementation
├── storage.py                # Context Persistence (SQLite)
├── features.py               # Feature Engineering (Regex/Transforms)
├── config.py                 # Pydantic RouterConfig
├── models.json               # Model registry (35 models)
├── utils/
│   ├── calibration.py        # Sigmoid Normalization Logic
│   └── warmup.py             # Procedural Warmup Logic
└── priors/
