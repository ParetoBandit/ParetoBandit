...
constraints may be needed for high-stakes applications.  The $K{=}10$
multi-model results (Section~5.2) underscore this concern: at
$\lambda{=}1$, the router achieves 89\% cost reduction versus
static GPT-4.1 by routing the vast majority of traffic to cheaper
models, which amplifies the risk that certain user populations are
systematically served by lower-capability backends.
...
The catastrophic failure experiments
(Appendix~\ref{appendix:catastrophic_failure}) demonstrate that Corralling
can automatically redistribute traffic away from a failing model (95\%
detection rate at $K{=}5$), which reduces user exposure to harmful
outputs --- but this re-routing occurs without human intervention,
compounding the accountability question for operators who must explain
after the fact why different users received different models.
...
\paragraph{Ethics of dynamic model onboarding.}
The \texttt{register\_model()} API
(Appendix~\ref{appendix:model_onboarding}) allows operators to add new
models to a running router without restarting or retraining.  The newcomer
inherits reward estimates via semantic bootstrapping (nearest-neighbor
$\thetavec$ transfer) and, when applicable, family parameter sharing
(inherited $\betavec$), which together reduce initial prediction error by
up to 46\% and allow the newcomer to receive real user traffic within its
first routing decision.  While this enables rapid fleet evolution, it
raises transparency and validation concerns: users are unaware that a
new, potentially less-tested model has entered the fleet, and family
sharing means the newcomer inherits biases encoded in the family's shared
representation before accumulating corrective observations of its own.

\emph{Recommendation:}  Operators should gate model onboarding behind a
validation step that confirms the newcomer meets minimum quality and
safety thresholds on a held-out evaluation set before enabling it for
production traffic.  The \texttt{explain\_decision} API can be used to
monitor the newcomer's traffic share and per-prompt confidence during
the burn-in period.  For safety-critical deployments, consider restricting
the newcomer to a shadow mode (logging routing decisions without serving
them) until sufficient observations confirm alignment with the fleet's
...
experiments (1,400+ architectural ablation runs, 360 PCA-vs-$K$ runs,
catastrophic failure simulations across $K{\in}\{2,3,5,10\}$, model
onboarding trials, hyperparameter sweeps, learning curves, and
statistical tests --- over 2,000 individual runs in total) was under
10 minutes on a single Apple M2 laptop.  The embedding computation
(SentenceTransformer) adds approximately 4 seconds per experiment.  We
consider the environmental footprint of the experimental evaluation to
be minimal.
