...
\midrule
banditGPT vs.\ RouteLLM-MF & $+0.033$ & 25.91 & $2.8 \times 10^{-16}$ & 5.79 & $8.6 \times 10^{-5}$ \\
banditGPT vs.\ Static Mixtral & $+0.093$ & 7.12 & $2.6 \times 10^{-12}$ & 0.28 & $3.2 \times 10^{-10}$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Bootstrap CI.}
The 95\% bootstrap CI for banditGPT's mean reward is $[0.913, 0.918]$,
which entirely excludes RouteLLM-MF's best point ($0.883$).

\paragraph{Oracle gap closure.}
banditGPT closes $\frac{0.914 - 0.823}{0.953 - 0.823} = 70.0\%$ of the
Oracle--Mixtral gap, vs.\ RouteLLM-MF's 46.2\%.

\subsubsection{Interpretation}

\paragraph{1.\ Large, consistent advantage over RouteLLM-MF.}
Cohen's $d = 5.79$ is far above the ``large'' threshold ($d \geq 0.8$),
and every one of banditGPT's 20 trial means (range: 0.904--0.925) exceeds
RouteLLM-MF's best threshold reward (0.883).  The 95\% bootstrap CI
$[0.913, 0.918]$ entirely excludes the RouteLLM-MF reference, providing
...
even the lower bound (0.913) exceeds the RouteLLM-MF reference by 3.0
