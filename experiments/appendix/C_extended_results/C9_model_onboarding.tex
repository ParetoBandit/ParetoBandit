...
\midrule
Shared family   & 0.970 & $\pm$0.002 & 430 $\pm$ 29 \\
Isolated family & 0.971 & $\pm$0.003 & 478 $\pm$ 18 \\
\midrule
\textbf{Difference} & $-$0.001 & & $-$48 \\
\bottomrule
\end{tabular}
\end{table}

\noindent A paired $t$-test on per-trial holdout rewards confirms no
significant difference: $t(19){=}{-}0.87$, $p{=}0.394$, with 8 of 20
...
\begin{enumerate}[nosep]
    \item \textbf{Family sharing does not improve final quality.}
          ...
          significant holdout quality advantage ($p{=}0.394$).
    ...
    \item \textbf{Both conditions succeed comprehensively.}
          Holdout reward ($0.970$) substantially exceeds the 2-model
          baseline ($0.903$, Table~\ref{tab:learning_curve}) and
...
$0.903$ to $0.970$ (+6.7~pp), and this improvement was identical
regardless of whether family sharing was active ($p{=}0.394$).
...
Section~\ref{sec:transfer_null} reports that one-shot per-model
semantic $\thetavec$ transfer provides no statistically significant
benefit beyond the continuous family $\betavec_F$ sharing of Hybrid LinUCB
($p{>}0.5$, Cohen's $d{<}0.05$ at $K{=}10$).  In our evaluation, this suggests the hybrid
decomposition is an effective abstraction level for cross-model knowledge
transfer: the family $\betavec_F$, updated with \emph{every} observation
from any family member, subsumes the one-shot $\thetavec$ initialization.
The current experiment evaluates \emph{dynamic} onboarding with
\texttt{register\_model()}, where the critical ingredient is the
confidence reset ($\Amat{=}\lambda\mathbf{I}$): it gives the newcomer a
comparable UCB exploration bonus to established models, allowing the
router to learn the newcomer's true quality within a few hundred
observations.  The family $\betavec_F$ provides the warm start; the
confidence reset enables exploration.
