...
\paragraph{K=3 portfolio (\chNPromptsK{} val prompts; sanity check).}
...
\paragraph{K=4 portfolio (\chNPromptsKFour{} val prompts, with Gemini-Flash).}
Adding a fourth arm whose pricing overlaps an existing arm provides
a harder test of the heuristic.
The K=4 split contains \chNPromptsKFour{} of the \chNPromptsK{} val prompts;
\chKFourMissing{} prompts are absent from the K=4 data file (excluded during
Flash data collection, likely due to API refusals or empty responses).
...
Three structural properties of the routing architecture limit the
downstream impact of these inversions:
\begin{enumerate}[nosep,leftmargin=*]
  \item $\tilde{c}_a$ enters \emph{only} the soft penalty
        (Eq.~\ref{eq:budget_ucb}); the hard ceiling
        and the budget pacer's EMA cost signal both operate on actual
        per-request costs, so heuristic errors cannot cause budget
        violations.
  \item For closely priced arms, the absolute penalty gap
        $(\lambda_c + \lambda_t)\,\Delta\tilde{c}$ is small.
        At baseline ($\lambda_t{=}0$), the Mistral--Flash gap
        contributes only $0.3 \times \chMistralFlashGap \approx 0.015$
        to the score---negligible relative to the $[0,1]$ reward
        signal that dominates arm selection.
  \item The dual variable $\lambda_t$ updates on \emph{actual} costs
        (Eq.~\ref{eq:dual_update}).  If heuristic error causes
        persistent over-selection of a costly arm, the resulting
        spending increase drives $\lambda_t$ upward, tightening
        the cost penalty for all expensive arms until compliance
        is restored.
\end{enumerate}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../experiments/appendix/cost_heuristic_validation/results/cost_heuristic_validation.png}
\caption{Cost heuristic validation.  Per-request realised cost distributions for the $K{=}3$ and $K{=}4$ portfolios, with the heuristic ordering overlaid.  The static log-normalized cost $\tilde{c}_a$ preserves the true per-request cost ranking on 100\% of $K{=}3$ prompts and ${\sim}80\%$ of $K{=}4$ prompts (the Mistral-Large vs.\ Gemini-Flash pair is the only source of inversions).}
