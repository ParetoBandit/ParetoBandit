...
All three conditions share $\alpha{=}\waAlpha$; the
conditions differ only in prior strength
($n_\mathit{eff}{=}\waNeff$ for warmup vs.\ $1$ for both cold
starts) and forgetting factor ($\gamma{=}\waGamma$ for warmup
and the matched control vs.\ $\gamma{=}\waTRGamma$ for Tabula
Rasa's independently optimised configuration).
