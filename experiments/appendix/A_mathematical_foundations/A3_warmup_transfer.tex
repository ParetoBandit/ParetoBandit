...
precision matrix and the moment vector by $\neff$, preserving the mean
prediction $\hat{\theta} = \theta_{\text{neighbor}}$ regardless of $\neff$.
...
at short creative prompts but struggles with long creative prompts).  Disjoint
LinUCB provides per-arm linear fits, which offer piecewise flexibility across
models, but cannot capture within-arm non-linearity.  Kernel or neural
extensions~\cite{zhou2020neural} could address this at the cost of
