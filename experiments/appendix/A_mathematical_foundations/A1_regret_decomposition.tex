...
architecture: Hybrid LinUCB experts coordinated by a Corralling
meta-learner (Log-Barrier OMD)~\cite{agarwal2017corralling}.  Each expert decomposes the reward
into a family-shared component and an arm-specific residual
(Algorithm~\ref{alg:linucb}); we state known results from the literature and
show how they compose to yield the system-level bound that our experiments
validate.
...
We assume a linear realizability condition.  Under the default Hybrid LinUCB
policy, the reward for arm $a$ in family $g = F(a)$ decomposes as:
\begin{equation}
\mathbb{E}[r_t(a) \mid x_t] = x_t^\top \beta_g^* + x_t^\top \theta_a^*
\quad \forall\, a \in [K]
\label{eq:hybrid_reward}
\end{equation}
where $\beta_g^* \in \mathbb{R}^d$ is the family-shared reward parameter and
$\theta_a^* \in \mathbb{R}^d$ is the arm-specific residual.  Defining
$\phi_a^* \triangleq \beta_{F(a)}^* + \theta_a^*$, this is equivalent to the
standard per-arm linear model:
\begin{equation}
\mathbb{E}[r_t(a) \mid x_t] = x_t^\top \phi_a^* \quad \forall\, a \in [K]
\label{eq:linear_reward}
\end{equation}
The pseudo-regret is defined as
$R(T) = \sum_{t=1}^T \bigl[ \max_{a} x_t^\top \phi_a^* - x_t^\top \phi_{a_t}^* \bigr]$.
...
reward surfaces need not be exactly linear in the PCA features.  The Hybrid
LinUCB policy learns both a family-shared $\beta_g$ and a per-arm residual
$\theta_a$, which provides piecewise-linear flexibility across models and
enables within-family transfer, but curvature within each arm's response
surface is not captured.  Section~\ref{appendix:warmup_limitations} discusses
this limitation and its practical impact.
\end{remark}

\subsubsection{Level 1: Per-Expert Regret (Hybrid LinUCB)}

Each expert in the banditGPT system is an instance of Hybrid LinUCB
(Algorithm~\ref{alg:linucb}) with ridge regularization $\lambda > 0$ and
exploration coefficient $\alpha$.  Because the hybrid decomposition
$\phi_a^* = \beta_{F(a)}^* + \theta_a^*$ yields per-arm effective parameters
$\phi_a^*$, the standard Disjoint LinUCB regret bound applies to the combined
predictions.  The following result is
standard~\cite{li2010contextual, abbasi2011improved}:
...
$\|x_t\| \leq 1$, $\|\phi_a^*\| \leq S$ for all arms~$a$, and sub-Gaussian
noise with parameter $\sigma$, the LinUCB algorithm with
...
In our system, two such experts operate in parallel.  Both use the Hybrid
LinUCB architecture: a family-shared $\beta_g$ (pooling data across arms in
the same model family) and an arm-specific residual $\theta_a$.  When a
\texttt{family\_map} is provided, models from the same provider (e.g.,
GPT-4-Turbo and GPT-4o) share a common $\beta_g$, enabling transfer when new
models join an existing family.
\begin{itemize}[leftmargin=*,noitemsep]
    \item \textbf{Expert 1 (Warmup):} \texttt{CostAwareLinUCBRouter} initialized
    with pre-trained arm-specific priors from 80k LMSYS Arena battles.
    The prior matrices $A_a, b_a$ encode a warm start that effectively skips
    the initial exploration phase; the family-shared matrices $A_g^{(0)},
    b_g^{(0)}$ start from identity and accumulate during online learning
    (Section~\ref{appendix:warmup_transfer}).
    \item \textbf{Expert 2 (Tabula Rasa):} \texttt{CostAwareTabulaRasaRouter}
    initialized with $A_a = \lambda I$, $b_a = 0$ and
    $A_g^{(0)} = \lambda I$, $b_g^{(0)} = 0$ (no prior knowledge).
\end{itemize}

Both experts satisfy Theorem~\ref{thm:linucb_regret} individually via the
combined parameter $\phi_a^* = \beta_{F(a)}^* + \theta_a^*$.  The warmup
expert's prior can reduce the effective regret when accurate, but may increase
it under domain mismatch.

\subsubsection{Level 2: Meta-Regret (Corralling via Log-Barrier OMD)}

The Corralling meta-learner~\cite{agarwal2017corralling} maintains a
probability distribution $p^{(t)} \in \Delta^N$ over $N$ experts and selects
expert~$i$ with probability $p_i^{(t)}$ at each round.  The implementation
uses Online Mirror Descent (OMD) with a log-barrier regularizer and
per-expert adaptive learning rates, combined with a mixing
parameter~$\gamma$ for an explicit exploration floor:

\begin{equation}
p_i^{(t)} = (1 - \gamma)\,\frac{w_i^{(t)}}{\sum_j w_j^{(t)}} + \frac{\gamma}{N},
\qquad
w_i^{(t+1)} = \frac{w_i^{(t)}}{1 + \eta_i^{(t)}\,\hat{\ell}_i^{(t)}\,w_i^{(t)}}
\label{eq:corralling_update}
\end{equation}

where $\eta_i^{(t)} = \eta_0 / \sqrt{S_i^{(t)} + \epsilon}$ is the adaptive
learning rate for expert~$i$, $S_i^{(t)} = \sum_{\tau=1}^{t-1}
(\hat{\ell}_i^{(\tau)})^2$ is the cumulative squared loss,
$\hat{\ell}_i^{(t)} = \ell_i^{(t)} / p_i^{(t)}$ is the importance-weighted
loss for endorsing experts ($\hat{\ell}_j^{(t)} = 0$ for non-endorsing $j$),
and $\ell_i^{(t)} = 1 - r_t$.
